...
model = PPO(
        ...
        learning_rate = 2.5e-4, # set the rate in which the model updates weight values, with 1e-4 as a smaller, more basic and stable rate.
        ...
        ent_coef = 0.005, # set reduced entropy coefficient to a lower value 0.005 (lower penalty for certainty, prioritizes stability and exploitation over exploration)
...
multi_batch_amnt = 4 # the batchs for the multi-batch approach
...
for update in range(total_updates):
    ...
    data_done = False # reset the isDone status or completion status
    final_rewards = []
    ...
    for _ in range(multi_batch_amnt):
        ...
        for _ in range(batch_size):
            ...
            augmented_rewards.append(current_reward)
...
model.save("outputs/phase3-model")
env.save("outputs/stats3.pkl")

# evalute model over different episode amounts
episode_amounts = [100, 1000, 5000]
with open("outputs/phase3.txt", "w") as FILEIN:
    FILEIN.write(f"Phase 3 (OPPO+ Algorithm) training completion time: {training_time:.3f} seconds\n")
